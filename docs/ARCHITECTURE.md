...
After core freeze, the runtime adds four coordinated layers: protected rules/core, per-task SPEC/PLAN, canonical `.idea-to-build/tasks.json`, and configured quality gates with snapshot-bound evidence. `docs/live/TASKS.md` is generated from JSON and never becomes a second state source. Task writes deep-copy and fully validate the candidate ledger before replacing JSON; create and block operations validate the complete proposed change before persistent mutation, and `project_state.json.planned_tasks` is resynchronized from non-terminal tasks. These safeguards do not turn the ledger, project state, and Markdown projection into one cross-file transaction; JSON remains canonical and the projection is rebuildable. `render_context()` selects one task and emits bounded, data-delimited summaries; parallel mode refuses to guess. Stop uses a lightweight path for memory-only maintenance and a task-aware path for substantive changes.
