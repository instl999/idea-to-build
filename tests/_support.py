...
class ProjectFixture:
    def __init__(self, name="Example Product"):
        self.temp = tempfile.TemporaryDirectory(dir=str(TEST_TEMP))
...
def run_hook(script_name, event):
    ...
    result = subprocess.run([sys.executable, str(ROOT / "hooks" / script_name)], input=json.dumps(event), text=True, capture_output=True)
