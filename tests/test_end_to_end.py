...
class EndToEndTests(unittest.TestCase):
    def test_init_ready_freeze_generate_verify(self):
        ...
        try:
            ...
            handoff = itb.generate_handoff(fx.root); self.assertEqual(handoff["thread_count"], 1)
            self.assertFalse(handoff["subagents_recommended"])
            self.assertEqual(itb.load_state(fx.root)["codex_dispatch_status"], "NOT_NEEDED")
