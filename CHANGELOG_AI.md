...
- 远端证据：提交 `f3d51b8` 的 push run `31089369189` 在 Ubuntu 3.9 的 `tearDown` 因临时 Git 子目录消失而失败；首轮夹具修复提交 `8c7d616` 的 PR run `31090642760` 六项全绿，但 push run `31090639021` 的 Ubuntu 3.9/3.13 又显示删除中的 `project` 或 `.git` 被重新写入，`rmdir` 返回 `ENOTEMPTY`。两次均由 `gh run view --log-failed` 定位。
- 根因与最小修复：测试夹具原先在删除前递归预遍历，且第一次修复只覆盖路径消失，没有覆盖目录在删除过程中重新变为非空的竞态。最终实现只对 `ENOTEMPTY`/`EEXIST` 做最多八次、总等待不超过 1.4 秒的整树重试；只读权限与路径消失单独处理，重试耗尽和其他 I/O 错误继续抛出。生产运行时、CLI、schema、核心冻结和调度契约均未改变。
- 回归与恢复：新增确定性测试覆盖瞬态成功、重试耗尽和非瞬态错误传播；定向失败用例在 Windows 连续运行 20 次通过。完整隔离验证为 122 项测试，121 项通过、1 项环境性跳过，编译、包校验、冻结示例和发布审计也通过；若行为不符可回退夹具提交，不影响产品运行时数据。
